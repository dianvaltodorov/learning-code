...
;Зад.1. Да се напише функция (squares-product a b), която изчислява произведението на всички точни квадрати в даден целочислен интервал [a;b]. Hint: използвайте вградения предикат (Int? n).
;(squares-product 1 10) -> 36   ; =1*4*9 = 1^2+2^2+3^2
;(squares-product 1000 1100) -> 1115136   ; =1115136 = 10562
(define (squares-product a b)
  (accumulate *
              1
              (lambda (i) (if (Int? (sqrt i)) i 1))
              a
              (lambda (i) (+ i 1))
              b))
